...
class Ship < Entity
  ...
  getter :health, :docking_status, :planet
  ...
  @y : Float64
  @health : Int32
  @docking_status : Int32
  @docking_progress : Int32
  @planet_id : (Int32)?
  @planet : (Planet)?
  ...
  def initialize(player_id, ship_id, x, y, hp, status, progress, planet_id)
    ...
    @planet_id = planet_id if @docking_status != DockingStatus::UNDOCKED
  end
  ...
  def thrust(magnitude, angle)
    "t #{id} #{magnitude.to_i} #{angle.round.modulo(360)}"
  end
  ...
  def self.parse(player_id, tokens)
    ships = {} of Int32 => Ship
    count_of_ships = tokens.shift.to_i
  ...
  def self.parse_single(player_id, tokens)
    # The _ variables are deprecated in this implementation, but the data is still # being sent from the Halite executable.

    id = tokens.shift.to_i
    x = tokens.shift.to_f64
    y = tokens.shift.to_f64
    hp = tokens.shift.to_i
    _velocity_x = tokens.shift
    _velocity_y = tokens.shift
    status = tokens.shift.to_i
    planet_id = tokens.shift.to_i
    progress = tokens.shift.to_i
    _weapon_cooldown = tokens.shift

    # player_id, ship_id, x, y, hp, docking_status, progress, planet_id
    ship = Ship.new(player_id, id, x, y, hp, status, progress, planet_id)
