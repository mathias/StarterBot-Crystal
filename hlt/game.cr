...
class Game
  ...
  getter :name, :logger, :map

  @name : String
  @logger : Logger
  @map : Map

  def initialize(name_of_bot)
    # implicit IO flush
    #$stdout.sync = true

    @name = name_of_bot
    player_id = read_from_input.to_i
  ...
  private def set_up_logging(player_id)
    io = File.new("#{player_id}_#{name}.log")
    logger = Logger.new(io)
    logger.formatter = Logger::Formatter.new do |severity, datetime, progname, message, io|
      io << "#{datetime.to_s("%m-%d %H:%M:%S.%3N")} - #{severity.to_s} - #{message}\n"
    end
    logger
  end

  private def read_from_input
    STDIN.read_line.strip
  end

  private def read_ints_from_input
    read_from_input.split(' ').map { |v| v.to_i }
  end

  private def write_to_output(data)
    data = "#{data.strip}\n"
